...
public final class JISession {
			...
			try {
				while (true) {
					...
					if (r != null) {
						...
						// does not work, since
						// session is strongly referenced from
						// mapOfSessionIdsVsSessions and listOfSessions and even
						// putting
						// WeakReference for JISession when adding it to the
						// mapOfSessionIdsVsSessions/listOfSessions does not
						// make a difference as we always loose the session to
						// GC before it come here.
						if (holder.isOnlySessionIDPresent) {
							try {
								...
								if (JISystem.getLogger().isLoggable(Level.FINEST)) {
									JISystem.getLogger().finest(
											"exception from destroy session in clean up thread: " + e.getMessage());
								}
							}
						} else {
							...
							try {
								...
								// Since we are freeing up all references for
								// the given IPID together, ensure
								// that all weak-references for this IPID have
								// been dereferenced before it to
								// the list of Dereferenced IPIDs. The Reference
								// Queue mechanism ensures that
								...
								// Will call the JIComOxidRuntime, and that is
								// synched on mutex3, but that will not cause a
								// deadlock, since
								// it or rather any method of JIComOxidRuntime
								// does not call back into JISession.
								JIComOxidRuntime.delIPIDReference(IPID, new JIObjectId(holder.oid, false), session);

								// Only proceed to de-list this IPID for
								// clearance if all weak-references were
								// released.
								if (weakRefsRemaining > 0)
									continue;

								// JIComOxidRuntime.delIPIDReference(IPID);
								// session.releaseRef(IPID); Not doing release
								// anymore, this causes a lot of calls to
								// go across, so will save these in this list
								// and then the cleanup thread will deal with
								// this every 3 minutes.
								if (JISystem.getLogger().isLoggable(Level.FINEST)) {
									JISystem.getLogger().finest(
											"Adding Dereferenced IPID " + IPID + " session "
								...
								if (JISystem.getLogger().isLoggable(Level.INFO)) {
									JISystem.getLogger().info(
											"exception from removing a IPID from session in clean up thread: "
													+ e.getMessage());
								}
							}
						}
	...
	// Added by Ge Yun Peng to support webapp ServletContextListener call on
	// webapp shutdown (not JVM shutdown)
	...
	static {
		...
		System.setProperty("jcifs.smb.client.domain", "JIDomain");// is being
																	// put in
																	// for
																	// completing
																	// type2
		...
		cleanUpThread.setName("vBox JISession Reference Cleanup Thread");
		...
		shutdownThread = new Thread(new Runnable() {
			public void run() {
				Thread.currentThread().setName("vBox JISession Shutdown Thread");
				int i = 0;
	...
	/**
	 * Cancels the existing timer used to schedule collection of un-referenced
	 * COM Objects and then restarts the same with the new frequency. Default
	 * timer schedules the GC task every 2 mins.
	 * 
	...
	private static class Release_References_TimerTask extends TimerTask {
		public void run() {
			try {
				// Use a clone so we dont hold on to the mutex for longer than
				// required.
				...
				while (i < listOfSessionsClone.size()) {
					...
					if (JISystem.getLogger().isLoggable(Level.INFO)) {
						JISystem.getLogger().info(
								"Release_References_TimerTask:[RUN] Ipid Vs Count Map size "
										+ session.mapOfIPIDsVsRefcounts.size() + " listOfDeferencedIpids size "
										+ session.listOfDeferencedIpids.size());
						JISystem.getLogger().info(
								"Release_References_TimerTask:[RUN] Session:  " + session.getSessionIdentifier()
										+ " , listOfDeferencedIpids: " + session.listOfDeferencedIpids);
					}

					// now iterate over each sessions listOfDereferencedIpids
					// and send a call to release for the entire lot.
					ArrayList listToKill = new ArrayList();
					List dereferencedIpids = null;

					// Use a clone so we dont hold on to the mutex for longer
					// than required.
					synchronized (mutex) {
					...
					for (int j = 0; j < dereferencedIpids.size(); j++) {
						try {
							...
							if (JISystem.getLogger().isLoggable(Level.INFO)) {
								JISystem.getLogger().info(
										"Release_References_TimerTask:[RUN] Exception preparing for release " + e);
							}
						}
					}
					synchronized (mutex) {
					...
					if (JISystem.getLogger().isLoggable(Level.INFO)) {
						JISystem.getLogger().info(
								"Release_References_TimerTask:[RUN] Ipid Vs Count Map size after preparing release "
										+ session.mapOfIPIDsVsRefcounts.size());
					}
					...
					if (listToKill.size() > 0) {
						...
						try {
							...
							JISystem.getLogger().logp(Level.SEVERE, "JISession", "Release_References_TimerTask:run()",
									"Exception in internal GC", e);
						}
					}
	...
	void setTargetServer(String targetServer) {
		if (targetServer.equalsIgnoreCase("127.0.0.1")) {
			// Replace with it's actual bindings, otherwise does not work for
			...
			if (localhostStr.equalsIgnoreCase("127.0.0.1") || localhostStr.equalsIgnoreCase("0.0.0.0")) { // Bug
																											// in
																											// JDK
																											// ,
																											// time
	...
	public static JISession createSession(IJIAuthInfo authInfo) {
		...
		session.sessionIdentifier = authInfo.getUserName().hashCode() ^ authInfo.getPassword().hashCode()
				^ authInfo.getDomain().hashCode() ^ new Object().hashCode() ^ (int) Runtime.getRuntime().freeMemory()
				^ randomGen.nextInt();
	...
	public static JISession createSession(String domain, String username, String password) {
		...
		session.sessionIdentifier = username.hashCode() ^ password.hashCode() ^ domain.hashCode()
				^ new Object().hashCode() ^ (int) Runtime.getRuntime().freeMemory() ^ randomGen.nextInt();
	 ...
	 * parameter. The new session is not yet attached to a COM server.
	 * 
	 * @param session
	 * @return
	 * @see JIComServer#JIComServer(JIClsid, JISession)
	...
	public static JISession createSession() {
		...
		JISession session = new JISession();
		session.sessionIdentifier = new Object().hashCode() ^ (int) Runtime.getRuntime().freeMemory()
				^ randomGen.nextInt();
		session.isSSO = true;
	...
	public static void destroySession(JISession session) throws JIException {
		...
		// if stub is null then cleanup datastructures holding the session
		// object only
		...
		try {
			...
			synchronized (mapOfObjects) {
				// now take all the objects registered with this session and
				// call release on them.
				// Iterator iterator = mapOfObjects.keySet().iterator();
				Iterator iterator = mapOfObjects.entrySet().iterator();
				while (iterator.hasNext()) {
					// String ipid =
					// (String)session.mapOfObjects.get(iterator.next());
					Entry entry = (Entry) iterator.next();
					// IPID_SessionID_Holder holder =
					// (IPID_SessionID_Holder)mapOfObjects.get(iterator.next());
					...
					// Commenting the line below since there could be more than
					// one reference of a COM object taken in by
					// j-Interop (via the client of j-Interop) and mapOfObjects
					// will contain two references in this case.
					// This was identified for the issue reported by Aquafold in
					// sql dbg.
	...
	private static void postDestroy(JISession session) throws JIException {
		...
		if (JISystem.getLogger().isLoggable(Level.INFO)) {
			JISystem.getLogger().info(
					"About to destroy links for Session: " + session.getSessionIdentifier() + " , size of which is "
							+ session.links.size());
		}
	...
	void setStub2(JIRemUnknownServer stub) {
		this.stub2 = stub;
		// no need to add this to the Oxid vs Sessions map as we would be using
		// the same interface pointer as the other stub.
	}
	...
	void addToSession(IJIComObject comObject, byte[] oid) {
		...
		addToSession(comObject.getIpid(), oid, ((JIStdObjRef) comObject.internal_getInterfacePointer()
				.getObjectReference(JIInterfacePointer.OBJREF_STANDARD)).getFlags() == 0x00001000);
		if (JISystem.getLogger().isLoggable(Level.INFO)) {
			JISystem.getLogger().info(
					" for IID: " + comObject.getInterfaceIdentifier() + " session: " + getSessionIdentifier());
		}

		int refcount = ((JIStdObjRef) comObject.internal_getInterfacePointer().getObjectReference(
				JIInterfacePointer.OBJREF_STANDARD)).getPublicRefs();
	...
	void addWeakReference(IJIComObject comObject, byte[] oid) {
		IPID_SessionID_Holder holder = new IPID_SessionID_Holder(comObject.getIpid(), getSessionIdentifier(), false,
				oid);
		synchronized (mapOfObjects) {
	...
	/*
	 * Reduce the count of weak-references stored in mapOfIPIDsVsWeakReferences
	 * and return the same.
	 */
	...
	private void addToSession(String IPID, byte[] oid, boolean dontping) {
		...
		// mapOfObjects.put(new
		// WeakReference(IPID,referenceQueueOfCOMObjects),IPID);
	...
	// {
	// JISystem.getLogger().warning("Dumping mapOfIPIDSvsCount " +
	// mapOfIPIDSvsCount.toString());
	// }
	// }

	// this gets called from the cleanupthread and no place else and it calls
	// the releaseRef of session which
	// internally calls the add_releaseRef of the JIComServer, that method is
	// synched at the instance level.
	// I was worried about a deadlock with destroySession , since that also
	// ultimately calls the add_releaseRef, but
	// this will not happen since under a simultaneous destroy and
	// removefromsession call , the "mutex" object will get synch.
	// If suppose a comServer.getInterface(...) is being done (which also calls
	// releaseRef), then that is synched at instance level
	// and so is add_releaseRef (on the same instance), so deadlock won't happen
	// there. If a simulataneous remove and getInterface call comes
	...
	void releaseRef(String IPID, int numinstances) throws JIException {
		if (JISystem.getLogger().isLoggable(Level.INFO)) {
			JISystem.getLogger().info(
					"releaseRef:Reclaiming from Session: " + getSessionIdentifier() + " , the IPID: " + IPID
							+ ", numinstances is " + numinstances);
		}
		...
		if (JISystem.getLogger().isLoggable(Level.INFO)) {
			JISystem.getLogger().warning(
					"releaseRef: Releasing numinstances " + numinstances + " references of IPID: " + IPID
							+ " session: " + getSessionIdentifier());
	...
	private void addDereferencedIpids(String IPID) {
		if (JISystem.getLogger().isLoggable(Level.INFO)) {
			JISystem.getLogger().info(
					"addDereferencedIpids for session : " + getSessionIdentifier() + " , IPID is: " + IPID);
		}
	...
	private void releaseRefs(JIArray arrayOfStructs, boolean fromDestroy) throws JIException {
		if (JISystem.getLogger().isLoggable(Level.INFO)) {
			JISystem.getLogger().info(
					"In releaseRefs for session : " + getSessionIdentifier() + " , array length is: "
							+ (short) (((Object[]) arrayOfStructs.getArrayInstance()).length));
		}
	...
	private JIStruct prepareForReleaseRef(String IPID) throws JIException {
		...
		int releaseCount = 5 + 5; // 5 of the original and 5 for the addRef done
									// later on.
		if (refcount != null)
	...
	private JIStruct prepareForReleaseRef(String IPID, int refcount) throws JIException {
		...
		if (JISystem.getLogger().isLoggable(Level.INFO)) {
			JISystem.getLogger().warning(
					"prepareForReleaseRef: Releasing " + refcount + "references of IPID: " + IPID + " session: "
	...
	/**
	 * Register handlers for OBJREF_CUSTOM. customClass only serves as a
	 * Template and is of no real consequence. A new copy is returned from
	 * customClass.decode(...) and that is used by framework internally.
	 * 
	 * @param CLSID
	 * @param customClass
	 */
